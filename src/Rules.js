...
class SmallStraight extends Rule {
  evalRoll = dice => {
    ...
    return d.size >= 4 && (!d.has(1) || !d.has(2) || !d.has(5) || !d.has(6))
      ? this.score
      : 0;
  };
}
...
const fullHouse = new FullHouse({
  score: 25,
  description: "Score 25 points if you have 2 and 3 of the same dice (ex:22444)"
});

// small/large straights score as 30/40
const smallStraight = new SmallStraight({
  score: 30,
  description: "Score 30 points if you have 4 ascending dice (ex: 2345)"
});
const largeStraight = new LargeStraight({
  score: 40,
  description: "Score 40 points if all 5 dice are ascending (ex: 12345)"
});

// yahtzee scores as 50
const yahtzee = new Yahtzee({
  score: 50,
  description: "Score 50 points if all 5 dice are the same number"
});
